...
5. **[fixed]** fish completions drift: `init --tmux-session` was missing; new
   commands added.
6. **[open, cosmetic]** The systemd units shipped in `systemd/` and the ones
   generated by `mcctl watchdog install` differ in description text and the
   `Documentation=` line (functionally identical). Single-source generation
   would prevent future drift.
7. **[open, edge]** `backup.create(full=True)` excludes
   `backup.remote_dir.lstrip('/')` from the tar — that exclude only matches if
   the backup dir is *inside* `server_dir`; with the default layout
   (`/opt/minecraft-backups` beside `/opt/minecraft`) it is inert (harmless,
   but the intent only holds for nested layouts).
8. **[open, minor]** `mcctl gui` forwards `--config` but not `-v` to the GUI
...
| doctor, props/jvm editing, spark profile, stats history, crash-report browser, sync | ✅ | ❌ deliberately CLI-only for now (rare, stateful or destructive); candidates for a future “Maintenance” page |
