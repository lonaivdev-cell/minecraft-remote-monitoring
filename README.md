...
| `mcctl items [list\|search QUERY\|icon ID]` | **EMI-style item index**: every item the pack defines with its real display name (from `en_us.json`) and resolved icon texture (model `parent`-chain → texture), read from the mod jars + `resourcepacks/`. `icon <id> -o out.png` downloads one item's PNG; the phone uses the same `items.manifest` + `icons.fetch` agent methods to cache icons and render recipes with pictures, offline |
