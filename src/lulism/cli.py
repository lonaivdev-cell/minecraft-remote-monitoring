...
def main(argv: list[str] | None = None) -> int:
    ...
    # migrate_legacy_dirs()'s own log.info() fired before any handler existed
    # (see above) and was dropped; re-announce here now that logging is live
    # so the migration is visible under -v.
    for src, dst in moved:
        log.info("migrated %s -> %s (original kept as rollback path)", src, dst)
